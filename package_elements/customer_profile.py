...
class SalesRow(object):
    ...
    def __init__(self, name, role, email, date, first_order, sales_total, total_orders, product_names,
        ...
        for i in information_repository.prep_p_list:
            for o in treated_product_list:
                if i in o:
                    products.append(i)
        self.product_list = products
        print(treated_product_list, self.product_list, len(self.product_list), len(self.product_quantity), len(self.product_item_cost))
        ...
        if self.date not in d.iat[6,1]:
            ...
            for i in range(len(self.product_list)):
                ...
                d.loc[d['Customer_Profile'] == product, 'Value'] = d.loc[d['Customer_Profile'] == product, 'Value'].astype(float) + float(quant)
                d.loc[d['Customer_Profile'] == product, 'Totals'] = d.loc[d['Customer_Profile'] == product, 'Totals'].astype(float) + float(price)
        else:
    ...
    def init_profile(self):
        profile_column = ['Name', 'Email', 'First_Order_Date', 'Gross_Sales', 'Number_of_Sales','Sales_Totals', 'Sales_Dates', 'Sales_Date_Differential'] + information_repository.p_list
        init_values =[self.name, (self.email), self.first_order, self.total_gross, self.total_orders,
                      None, None, None]
        end_values = list("0" * len(information_repository.prep_p_list))
