...
class SalesRow(object):
    ...
    def __init__(self, name, role, email, date,first_order, sales_total, total_orders, product_names,
        ...
        d.loc[7, 'Value'] += str(f'\n{self.date}')
        d.loc[5, 'Value'] = self.sales_total
        d.loc[6, 'Value']= self.total_orders
    ...
    def init_profile(self):
        profile_column = ['Name', 'Email', 'First_Order_Date', 'Sales_Total', 'Number_of_Sales', 'Sales_Dates'] + p_list
        init_values =[self.name, self.email, self.first_order, int(float(str(self.sales_total).replace(',', ''))),
                  int(self.total_orders), self.date]
